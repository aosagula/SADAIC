...
class WorkController extends Controller
{
    ...
    public function register(WorkRequest $request)
    {
        ...
        if ($registration) {
            ...
            if ($registration->audio_dnda_ed_file || $registration->audio_dnda_ed_file) {
    ...
    public function showList()
    {
        $requests = Registration::whereIn('id', function($query) {
            $query->select('registration_id') // Solicitudes donde el email del socio coincide con alguna de las partes
        ...
        ->orWhere('user_id', Auth::user()->id) // Solicitudes iniciadas por el socio
        ->orderBy('id', 'desc')
        ->get();
    ...
    public function deleteDistribution(Request $request)
    {
        try {
            ...
            // Si el registro no pertenece al usuario
            if ($distribution->registration->user_id != Auth::user()->id) {
                abort(403);
            }
    ...
    public function deleteRegistration(Registration $registration)
    {
        ...
        try {
            ...
            return redirect()->action('User\WorkController@showList')->with([
