...
<div class="container">
    <div class="row d-flex flex-row align-items-center">
        ...
        <div class="col col-3 text-right">
            <h3><a href="{{ url('/member/work/list') }}">Volver</a></h3>
        </div>
    </div>
    ...
    @php
        $response = $registration->distribution->where('member_id', Auth::user()->member_id)->first()->response;
    @endphp
    @if (!$response)
        @if ($response !== null)
        <div class="row pt-4">
...
window.onload = function() {
    $('#acceptDistribution').on('click', () => {
        axios.post('/member/work/{{ $registration->id }}/response', {
            response: 'accept'
        })
        .catch((err) => {
