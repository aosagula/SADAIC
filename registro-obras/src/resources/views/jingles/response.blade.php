...
<div class="container">
    <div class="row d-flex flex-row align-items-center">
        <div class="col col-9">
            <h2>Registro de Inclusión: {{ $registration->title }}</h2>
        </div>
    ...
    @php
        $response = $registration->agreements->where('member_idx', Auth::user()->sadaic->idx)->first()->response;
    @endphp
    @if (!$response)
        @if ($response !== null)
        <div class="row pt-4">
        ...
        <div class="row d-flex flex-row justify-content-around pt-4">
            <button class="btn btn-primary" id="acceptAgreement">Aceptar</button>
        </div>
        @else
        <div class="row d-flex flex-row justify-content-around pt-4">
            <button class="btn btn-primary" id="acceptAgreement">Aceptar</button>
            <button class="btn btn-danger" id="rejectAgreement">Rechazar</button>
        </div>
        @endif
    @else
        <div class="row pt-4">
...
window.onload = function() {
    $('#acceptAgreement').on('click', () => {
        axios.post('/{{ Auth::user()->type }}/jingles/{{ $registration->id }}/response', {
            response: 'accept'
        })
        .catch((err) => {
